...
var RedisAppender = module.exports = function(config) {
    ...
    this.performWrite = function(data) {
        ...
        if (!client) {
            return this.config.loglog('Write attempt to closed redis appender', self.config.host, self.config.port, self.config.namespace);
        }
        ...
        client.publish('loginator:' + self.config.namespace, data, function(err) {
            ...
            this.config.loglog('Unable to publish to', self.config.host, self.config.port, self.config.namespace, err);
