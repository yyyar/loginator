...
var Logger = function(config) {
    ...
    config = this.config = _.extend({}, configuration, config || {});
    ...
    this.appenders = _.map(this.config.appenders, function(a) {
        ...
        return new (APPENDERS[a.type]) (
            _.merge({}, a.options || {}, {level: a.level}, {loglog: config.loglog})
        );
    });
};
...
Logger.prototype.message = function(level, msg) {
    ...
    var context = {
        ...
        customVars: _.merge({}, this.config.customVars, this.extraVars||{})
    };
...
Logger.prototype.bind = function(vars) {
    ...
    boundLog.config = _.merge(_.cloneDeep(Object.getPrototypeOf(boundLog).config), {
        customVars: vars,
    });
    return boundLog;
};

Logger.prototype.with_remember = function(vars) {
    this.extraVars = vars;
    this.extraVarsOnce = false;
    return this;
}

Logger.prototype.with = function(vars) {
    this.extraVars = vars;
    this.extraVarsOnce = true;
    return this;
}
...
module.exports = {
    ...
    getLogger: function(name, config) {
        if (!_.includes(_.keys(loggers), name)) {
            loggers[name] = new Logger( _.merge(config || {}, {name:name}) );
        }
        return loggers[name];
    },
